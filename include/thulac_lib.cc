...
extern "C" int thulac_init(const char *model, const char *dict = NULL, int ret_size = 1024 * 1024 * 16, int t2s = 0, int just_seg = 0)
{
    if (ret_size < 4)
    {
        return int(false);
...
extern "C" void thulac_deinit()
{
    ...
    if (tagging_model != NULL)
    {
        ...
        {
            if (tagging_label_info)
                delete[](tagging_label_info[i]);
        }
    }
...
thulac_tag *_thulac_next_tag(struct thulac_tag *tag)
{
    tag->next = (struct thulac_tag *)calloc(1, sizeof(struct thulac_tag));
    return tag->next;
}

extern "C" struct thulac_tag *thulac_seg(const char *in, int bytes_len)
{
    if (bytes_len <= 0)
    {
        return NULL;
    ...
    struct thulac_tag result_head;
    struct thulac_tag *result_tag = &result_head;
        ...
        for (int vec_num = 0; vec_num < vec.size(); vec_num++)
        {
            ...
            if (raw.size())
            {
                ...
                if (seg_only)
                {
                    ...
                    {
                        ous << segged[j];
                        result_tag = _thulac_next_tag(result_tag);
                        result_tag->word = strndup(ous.str().c_str(), ous.str().length() + 1);
                        ous.str("");
                    }
                }
                else
                {
                    ...
                    //ous << tagged;
                    result_tag->next = (thulac_tag *)TaggedSentence::tags_out(tagged);
                }
            }
        }
    }
...
extern "C" void thulac_clean(struct thulac_tag *list) {
    while (list) {
        if (list->word) {
            free(list->word);
        }
